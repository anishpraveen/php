...
$sql = "SELECT emp_no, first_name, last_name, hire_date FROM employees ORDER BY emp_no DESC $limit";
$query = mysqli_query($conn, $sql);
// This shows the user what page they are on, and the total number of pages
$textline1 = "Employee List (<b>$rows</b>)";
...
while($row = mysqli_fetch_array($query, MYSQLI_ASSOC)){
	...
	$datemade = $row["hire_date"];
	$datemade = strftime("%b %d, %Y", strtotime($datemade));
	$list .= '<p>'.$id.' '.$firstname.' '.$lastname.' <br>Hired '.$datemade.'</p>';
}
